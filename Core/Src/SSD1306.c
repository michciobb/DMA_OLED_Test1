...
uint8_t GLCD_GetWidthChar(const char Character)
{
	...
	return (__GLCD.Font.Name[(Character - 32) * (__GLCD.Font.Width * __GLCD.Font.Lines + 1)] + 1);
}
...
static void GLCD_Send(const uint8_t Control, uint8_t *Data, const uint8_t Length)
{
	...
	do
	{
		...
		//Transmit SLA+W
		SSD1306_send_com(__I2C_SLA_W(__GLCD_I2C_Address));
		#if defined(GLCD_Error_Checking)
			status = __I2C_Status();
